...
if __name__ == '__main__' :
    ...
    for epoch in range(opt.epoch) :
        ...
        for n, sample in enumerate(tqdm(train_loader, desc="{:17s}".format('Training State'), mininterval=0.0001)) :
            ...
            # model: STIG_PGD 인스턴스, batch: 데이터 배치(딕셔너리)
            #print("before radius(frac):", torch.sigmoid(model.radius_param).item())
            #print("radius_param in optimizerG:", any(p is model.radius_param for g in model.optimizerG.param_groups for p in g['params']))
